...
class SpikingNN(torch.autograd.Function):
    def forward(self, input):
        ...
        return input.gt(1e-5).type(torch.FloatTensor)
...
class FlowNetS_spike(nn.Module):
    ...
    def forward(self, input, image_resize, sp_threshold):
        ...
        mem_1 = torch.zeros(input.size(0), 64, int(image_resize/2), int(image_resize/2))
        mem_2 = torch.zeros(input.size(0), 128, int(image_resize/4), int(image_resize/4))
        mem_3 = torch.zeros(input.size(0), 256, int(image_resize/8), int(image_resize/8))
        mem_4 = torch.zeros(input.size(0), 512, int(image_resize/16), int(image_resize/16))

        mem_1_total = torch.zeros(input.size(0), 64, int(image_resize/2), int(image_resize/2))
        mem_2_total = torch.zeros(input.size(0), 128, int(image_resize/4), int(image_resize/4))
        mem_3_total = torch.zeros(input.size(0), 256, int(image_resize/8), int(image_resize/8))
        mem_4_total = torch.zeros(input.size(0), 512, int(image_resize/16), int(image_resize/16))
