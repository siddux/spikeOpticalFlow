...
def train(train_loader, model, optimizer, epoch, train_writer):
    ...
    for ww, data in enumerate(train_loader, 0):
        ...
        if torch.sum(former_inputs_on + former_inputs_off) > 0:
            ...
            output = model(input_representation.type(torch.FloatTensor), image_resize, sp_threshold)
...
def validate(test_loader, model, epoch, output_writers,save_path):
        ...
        if torch.sum(former_inputs_on + former_inputs_off) > 0:
            ...
            if epoch < 0:
                ...
                if args.render:
                    ...
                    if args.save_images:
                        filename = str(i)+'.png'
                        directory = os.path.join(save_path,'spikeImages')
                        if not os.path.exists(directory):
                            os.makedirs(directory)
                        filename = os.path.join(directory,filename)
                        cv2.imwrite(filename,np.array(spike_image, dtype=np.uint8))
                ...
                if args.render:
                    ...
                    if args.save_images:
                        filename = str(i)+'.png'
                        directory = os.path.join(save_path,'grayImages')
                        if not os.path.exists(directory):
                            os.makedirs(directory)
                        filename = os.path.join(directory,filename)
                        cv2.imwrite(filename,cv2.cvtColor(gray, cv2.COLOR_BGR2RGB))
                ...
                if args.render:
                    ...
                    if args.save_images:
                        filename = str(i)+'.png'
                        directory = os.path.join(save_path,'predictedFlow')
                        if not os.path.exists(directory):
                            os.makedirs(directory)
                        filename = os.path.join(directory,filename)
                        cv2.imwrite(filename, cv2.cvtColor(flow_rgb, cv2.COLOR_BGR2RGB))
                ...
                if args.render:
                    ...
                    if args.save_images:
                        filename = str(i)+'.png'
                        directory = os.path.join(save_path,'gtFlow')
                        if not os.path.exists(directory):
                            os.makedirs(directory)
                        filename = os.path.join(directory,filename)
                        cv2.imwrite(filename, cv2.cvtColor(gt_flow_large, cv2.COLOR_BGR2RGB))
                ...
                if args.render:
                    ...
                    if args.save_images:
                        filename = str(i)+'.png'
                        directory = os.path.join(save_path,'predictedMaskedFlow')
                        if not os.path.exists(directory):
                            os.makedirs(directory)
                        filename = os.path.join(directory,filename)
                        cv2.imwrite(filename, cv2.cvtColor(flow_rgb_masked, cv2.COLOR_BGR2RGB))
                ...
                if args.render:
                    ...
                    if args.save_images:
                        filename = str(i)+'.png'
                        directory = os.path.join(save_path,'gtMaskedFlow')
                        if not os.path.exists(directory):
                            os.makedirs(directory)
                        filename = os.path.join(directory,filename)
                        cv2.imwrite(filename, cv2.cvtColor(gt_masked_flow, cv2.COLOR_BGR2RGB))
...
def main():
    ...
    if args.evaluate:
        with torch.no_grad():
            best_EPE = validate(test_loader, model, -1, output_writers, save_path)
        return
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if (epoch + 1)%args.evaluate_interval == 0:
            ...
            with torch.no_grad():
                EPE = validate(test_loader, model, epoch, output_writers, save_path)
